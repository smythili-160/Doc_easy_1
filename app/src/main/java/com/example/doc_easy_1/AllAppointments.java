...
public class AllAppointments extends AppCompatActivity {
    ...
    private void filterAppointmentsByInOrOut(String filterKeyword) {
        mAppointments.clear();
        adapter1.notifyDataSetChanged();
        appointments.collection("appointments").whereEqualTo("inOrOut", filterKeyword).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(QueryDocumentSnapshot document: queryDocumentSnapshots){
                    Appointment appointment = document.toObject(Appointment.class);
    ...
    private void filterAppointmentsByGender(String filterKeyword) {
        mAppointments.clear();
        adapter1.notifyDataSetChanged();
        appointments.collection("appointments").whereEqualTo("gender", filterKeyword).get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(QueryDocumentSnapshot document: queryDocumentSnapshots){
                    Appointment appointment = document.toObject(Appointment.class);
    ...
    private void showAllAppointments() {
        appointments.collection("appointments").get().addOnSuccessListener(new OnSuccessListener<QuerySnapshot>() {
            @Override
            public void onSuccess(QuerySnapshot queryDocumentSnapshots) {
                for(QueryDocumentSnapshot document: queryDocumentSnapshots){
                    Appointment appointment = document.toObject(Appointment.class);
                    String uniqueId = appointments.collection("appointments").document().getId();
    ...
    protected void onPostCreate(@Nullable Bundle savedInstanceState) {
        ...
        tb.setTitle("Appointment from post create");
    }
